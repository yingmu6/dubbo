...
/**
 * Different from {@link Filter}, ClusterInterceptor works at the outmost layer（最外层）, before one specific address/invoker is picked.
 */
@SPI
public interface ClusterInterceptor { // 与Filter不同，工作在最外层，对ClusterInvoker进行拦截处理
    ...
    default Result intercept(AbstractClusterInvoker<?> clusterInvoker, Invocation invocation) throws RpcException { //拦截调用：该方法不需要SPI实例重写，由框架自行调用，SPI实例只需重写before()、after()方法
        return clusterInvoker.invoke(invocation); //进入抽象类AbstractClusterInvoker执行调用，走通用逻辑
    }
