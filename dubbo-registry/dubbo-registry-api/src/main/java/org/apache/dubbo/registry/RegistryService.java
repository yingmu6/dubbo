...
public interface RegistryService { //注册、订阅时需要满足方法声明的契约
    ...
    /**
     * Register data（注册的数据）, such as : provider service, consumer address, route rule（路由规则）, override rule（覆盖规则） and other data.
     * <p>
     * Registering is required to support the contract（契约）:<br>
     * 1. When the URL sets the check=false parameter. When the registration fails, the exception is not thrown（异常不抛出） and retried in the background（后台重试）. Otherwise, the exception will be thrown.<br>
     * 2. When URL sets the dynamic=false parameter, it needs to be stored persistently（持久化存储）, otherwise, it should be deleted automatically when the registrant has an abnormal exit（异常退出）.<br>
     * 3. When the URL sets category=routers, it means classified storage（分类存储）, the default category is providers, and the data can be notified by the classified section. <br>
     ...
     * 5. When the registry is restarted and network jitter, it is necessary to automatically restore（恢复） the subscription request（订阅请求）.<br>
