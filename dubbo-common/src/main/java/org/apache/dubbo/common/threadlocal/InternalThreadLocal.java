...
public class InternalThreadLocal<V> { //与ThreadLocal具有相似的功能，都是维护线程局部变量的值
    ...
    private static void removeFromVariablesToRemove(InternalThreadLocalMap threadLocalMap, InternalThreadLocal<?> variable) {
        ...
        Set<InternalThreadLocal<?>> variablesToRemove = (Set<InternalThreadLocal<?>>) v;
        variablesToRemove.remove(variable); //set集合做指定值value移除（variablesToRemove改变，threadLocalMap的第一个集合元素也会对应改变）
    }
    ...
    public final void remove(InternalThreadLocalMap threadLocalMap) { //从InternalThreadLocalMap中移除指定的值
        ...
        Object v = threadLocalMap.removeIndexedVariable(index); //移除指定下标对应的值（即置为UNSET）
        removeFromVariablesToRemove(threadLocalMap, this); //将第一个元素对应的集合也移除对应的元素

        if (v != InternalThreadLocalMap.UNSET) {
            try {
                onRemoval((V) v); //看子类的具体实现
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
