...
public abstract class TreePathDynamicConfiguration extends AbstractDynamicConfiguration {
    ...
    /**
     * 发布配置（将配置发布到配置中心）
     * TreePathDynamicConfiguration有不同的实现：
     * 1）若是Zookeeper实现的ZookeeperDynamicConfiguration，就把配置在Zookeeper目录下创建一个节点
     * 2）若是本地文件实现的FileSystemDynamicConfiguration，就把内容写到本地文件中
     * 3）若是consul实现的配置中心ConsulDynamicConfiguration，就内容以key-value形式写到远程
     */
